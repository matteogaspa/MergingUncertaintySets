...
ml_g = lrn("regr.ranger", num.trees = 50, mtry = 4, min.node.size = 2)
ml_m = lrn("classif.glmnet", lambda = 1)
...
plot1 <- ggplot(data = data.plot, aes(x = k)) +
  ...
  theme_minimal() + theme(legend.position = "none", legend.title = element_blank()) + ylim(-0.35, 0.35) 
plot1
