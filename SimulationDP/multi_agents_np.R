...
cov50 <- l50 <- matrix(0, nrow = B, ncol = 14)
for(i in 1:B){
  samp <- runif(n)
  cis <- nppr_ci(nppr_algorithm(samp), r_t, alpha)
  cov50[i,k] <- I(cis[1] < 0.5 & 0.5 < cis[2])
  l50[i,k] <- cis[2]-cis[1]
  M_int <- matrix(NA, k, 2)
  M_int[k,] <- cis
  for(j in 1:(k-1)){
    rnd_obs <- sample(1:n, 0.5*n, replace = F)
    samp_plus <- c(samp[rnd_obs], runif(0.5*n))
    cis <- nppr_ci(nppr_algorithm(samp_plus), r_t, alpha)
    M_int[j,] <- cis
...
colMeans(cov50); colMeans(l50)

# Agent k share 75% of the data with agent k-1
cov75 <- l75 <- matrix(0, nrow = B, ncol = 14)
set.seed(1234)
for(i in 1:B){
  samp <- runif(n)
  cis <- nppr_ci(nppr_algorithm(samp), r_t, alpha)
  ...
  M_int[k,] <- cis
  for(j in 1:(k-1)){
    rnd_obs <- sample(1:n, 0.75*n, replace = F)
    samp_plus <- c(samp[rnd_obs], runif(0.25*n))
    cis <- nppr_ci(nppr_algorithm(samp_plus), r_t, alpha)
    M_int[j,] <- cis
...
l75[is.na(l75)] <- 0
colMeans(cov75); colMeans(l75)

# Agent k share 90% of the data with agent k-1
cov90 <- l90 <- matrix(0, nrow = B, ncol = 14)
set.seed(1234)
for(i in 1:B){
  samp <- runif(n)
  cis <- nppr_ci(nppr_algorithm(samp), r_t, alpha)
  ...
  M_int[k,] <- cis
  for(j in 1:(k-1)){
    rnd_obs <- sample(1:n, 0.9*n, replace = F)
    samp_plus <- c(samp[rnd_obs], runif(0.1*n))
    cis <- nppr_ci(nppr_algorithm(samp_plus), r_t, alpha)
    M_int[j,] <- cis
...
alpha <- 0.1
r_t <- (exp(2)-1)/(exp(2)+1)
cov <- lng <- matrix(0, nrow = B, ncol = 14)
set.seed(1234)
for(i in 1:B){
  samp <- runif(n*k/2)
  M_int <- matrix(NA, k, 2)
...
# agent
results <- matrix(NA, 8, 5)

results[1,1] <- lng[1,1]
results[2,1] <- colMeans(cov)[1:k]%>%mean

results[3,1] <- cov_ma$l50[1,1]
results[4,1] <- colMeans(cov50)[1:k]%>%mean

results[5,1] <- cov_ma$l75[1,1]
results[6,1] <- colMeans(cov75)[1:k]%>%mean

results[7,1] <- cov_ma$l90[1,1]
results[8,1] <- colMeans(cov90)[1:k]%>%mean

# majority vote
results[1,2] <- colMeans(lng)[k+1]
results[2,2] <- colMeans(cov)[k+1]

results[3,2] <- colMeans(cov_ma$l50)[k+1]
results[4,2] <- colMeans(cov50)[k+1]

results[5,2] <- colMeans(cov_ma$l75)[k+1]
results[6,2] <- colMeans(cov75)[k+1]

results[7,2] <- colMeans(cov_ma$l90)[k+1]
results[8,2] <- colMeans(cov90)[k+1]

# rnd majority vote
results[1,3] <- colMeans(lng, na.rm = T)[k+2]
results[2,3] <- colMeans(cov)[k+2]

results[3,3] <- colMeans(cov_ma$l50, na.rm = T)[k+2]
results[4,3] <- colMeans(cov50)[k+2]

results[5,3] <- colMeans(cov_ma$l75, na.rm = T)[k+2]
results[6,3] <- colMeans(cov75)[k+2]

results[7,3] <- colMeans(cov_ma$l90, na.rm=T)[k+2]
results[8,3] <- colMeans(cov90)[k+2]

# randomized union
results[1,4] <- colMeans(lng, na.rm = T)[k+3]
results[2,4] <- colMeans(cov)[k+3]

results[3,4] <- colMeans(cov_ma$l50, na.rm = T)[k+3]
results[4,4] <- colMeans(cov50)[k+3]

results[5,4] <- colMeans(cov_ma$l75, na.rm = T)[k+3]
results[6,4] <- colMeans(cov75)[k+3]

results[7,4] <- colMeans(cov_ma$l90, na.rm=T)[k+3]
results[8,4] <- colMeans(cov90)[k+3]

# exchangeable mv
results[1,5] <- colMeans(lng, na.rm = T)[k+4]
results[2,5] <- colMeans(cov)[k+4]

results[3,5] <- colMeans(cov_ma$l50, na.rm = T)[k+4]
results[4,5] <- colMeans(cov50)[k+4]

results[5,5] <- colMeans(cov_ma$l75, na.rm = T)[k+4]
results[6,5] <- colMeans(cov75)[k+4]

results[7,5] <- colMeans(cov_ma$l90, na.rm=T)[k+4]
results[8,5] <- colMeans(cov90)[k+4]



colnames(results) <- c("Ck", "CM", "CR", "CU", "CE")
